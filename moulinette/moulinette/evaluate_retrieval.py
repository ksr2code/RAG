# ABOUTME: Evaluation logic for retrieval quality in the RAG pipeline.
# ABOUTME: Computes recall@k by comparing student search results against ground-truth sources.
...
def calculate_recall_at_k_for_one_question(
    ...
    for i, true_source in enumerate(true_sources):
        # if true_source.file_path.endswith(".py"):
        #     continue
...
def build_eval_objects(
    student_search_results: StudentSearchResults,
    rag_dataset: RagDataset,
) -> Dict[str, EvalObject]:
    """Build a mapping of question_id -> EvalObject joining dataset and student results.

    Each dataset question gets an EvalObject with its true sources.
    Student predicted sources are matched by question_id. Questions without
    student results keep an empty pred_sources list.
    """
